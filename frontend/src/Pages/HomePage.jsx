      ...
      <div className="relative flex min-h-screen w-full flex-col">
        ...
        <header className="flex items-center justify-between px-6 py-6 max-w-7xl mx-auto w-full z-50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">ShortURL</h2>
          </div>
          
          <nav className="hidden md:flex gap-8 text-sm font-medium text-slate-600 dark:text-slate-400">
            <a className="hover:text-primary dark:hover:text-white transition-colors" href="#features">Features</a>
            <a className="hover:text-primary dark:hover:text-white transition-colors" href="#how-it-works">How it works</a>
            <Link className="hover:text-primary dark:hover:text-white transition-colors" to="/pricing">Pricing</Link>
          </nav>
          ...
          <div className="flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-4">
                ...
                <Link to="/dashboard" className="bg-primary text-white px-5 py-2.5 rounded-full text-sm font-semibold shadow-lg shadow-primary/20">
                  Dashboard
                </Link>
              </div>
            ) : (
              <>
                <Link className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors" to="/login">Log in</Link>
                <Link className="hidden sm:flex bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-slate-800 dark:hover:bg-slate-200 transition-all shadow-lg" to="/register">Sign up free</Link>
              </>
            )}
          </div>
        </header>
        ...
        <main className="flex-1 w-full relative overflow-hidden">
          ...
          <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center flex flex-col items-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-700 shadow-sm mb-8">
              ...
              <span className="text-xs font-medium text-slate-600 dark:text-slate-300">New: Enhanced analytics dashboard</span>
            </div>
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-6 leading-[1.1]">
              ...
              <span className="bg-gradient-to-r from-primary to-primary-dark bg-clip-text text-transparent">count.</span>
            </h1>
            
            <p className="text-lg sm:text-xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
              Create short, branded links that drive clicks and track performance.
              Join thousands of marketers creating millions of links each month.
            </p>
            ...
            <div className="w-full max-w-2xl mx-auto relative group z-10">
              ...
              <form onSubmit={handleShorten} className="relative bg-white dark:bg-surface-dark p-2 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700/50 flex flex-col sm:flex-row items-center gap-2">
              ...
              {shortenedUrl && (
                <div className="mt-4 animate-in fade-in slide-in-from-top-4 duration-500 bg-white dark:bg-surface-dark rounded-xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex flex-col text-left overflow-hidden w-full">
                    ...
                    <a className="text-primary font-medium hover:underline truncate" href={`https://${shortenedUrl}`} target="_blank" rel="noreferrer">{shortenedUrl}</a>
                  </div>
                  <div className="flex gap-2 w-full sm:w-auto">
                    ...
                    <button className="px-4 py-2 text-sm font-medium text-white bg-slate-900 hover:bg-slate-800 rounded-lg transition-colors">QR Code</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <p className="mt-6 text-sm text-slate-400 dark:text-slate-500">
              By clicking Shorten URL, you agree to our <Link className="underline hover:text-slate-600 dark:hover:text-slate-300" to="/terms">Terms of Service</Link>.
            </p>
          </section>
          {/* --- STATS SECTION --- */}
...
<section className="py-24 bg-transparent">
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div className="text-center mb-16">
      <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">Loved by marketing teams</h2>
      <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">Don't just take our word for it. Here's what our users have to say.</p>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      />
          ...
          <section id="how-it-works" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 mb-12 relative">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">How it works</h2>
              <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">Get started in seconds. No technical knowledge required.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
              <Step icon="content_paste" number="1" title="Paste Your Link" desc="Copy your long URL and paste it into our shortening tool." />
              <Step icon="auto_fix_high" number="2" title="Click Shorten" desc="Hit the button. Our system instantly generates a secure link." />
              <Step icon="rocket_launch" number="3" title="Share & Track" desc="Share your link everywhere and watch clicks roll in." />
            </div>
          </section>
          ...
          <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
            <div className="bg-slate-900 dark:bg-slate-800 rounded-[2.5rem] p-10 sm:p-16 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h2 className="text-3xl sm:text-4xl font-bold text-white mb-6">Ready to boost your connections?</h2>
                <p className="text-slate-300 text-lg mb-10 max-w-2xl mx-auto">Get started for free today and take control of your links.</p>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <Link to="/register" className="w-full sm:w-auto px-8 py-4 bg-primary hover:bg-primary-dark text-white rounded-xl font-semibold transition-all shadow-glow">
                    Get Started for Free
                  </Link>
                  <Link to="/contact" className="w-full sm:w-auto px-8 py-4 bg-white/10 hover:bg-white/20 text-white rounded-xl font-semibold backdrop-blur-sm transition-all border border-white/10">
                    Contact Sales
                  </Link>
                </div>
              </div>
        ...
        <footer className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-surface-dark py-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="flex items-center justify-center gap-3 mb-6">
              ...
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">ShortURL</h2>
            </div>
            <p className="text-sm text-slate-400">© 2026 ShortURL Inc. All rights reserved.</p>
          </div>
        </footer>
      </div>
    </div>
